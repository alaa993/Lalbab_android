...
public class My_order_adapter extends RecyclerView.Adapter<My_order_adapter.MyViewHolder> {
    ...
    public void onBindViewHolder(My_order_adapter.MyViewHolder holder, int position) {
        ...
        long timestamp = Long.parseLong(mList.getOn_date());


        holder.tv_date.setText(context.getResources().getString(R.string.date) +  getDate(timestamp).toString());
       // holder.tv_time.setText(context.getResources().getString(R.string.time) + mList.getDelivery_time_from() + "-" + mList.getDelivery_time_to());
        ...
        holder.tv_time.setText(""+getSeler(mList.getSeler_id()));
    ...
    private String getSeler(String seler_id){
        String ss = "";
        for (int i=0;i< BaseURL.category_modelList.size();i++){
            if (BaseURL.category_modelList.get(i).getId().equals(seler_id)){
                ss = BaseURL.category_modelList.get(i).getTitle();
                break;
            }
        }
        return ss;
